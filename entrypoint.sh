...
export MP3_LIMIT=${MP3_LIMIT:-"100"}
...
if [ "$RUN_SCHEDULED" = "True" ]; then
  ...
  echo "${CRON_SCHEDULE} /usr/local/bin/python3 /app/run.py --folder /app/music --mp3-limit ${MP3_LIMIT} >> /var/log/cron.log 2>&1" > /etc/cron.d/lyrics-cron
...
if [ "$RUN_ONCE" = "True" ]; then
  ...
  /usr/local/bin/python3 /app/run.py --folder /app/music --mp3-limit ${MP3_LIMIT}
fi
